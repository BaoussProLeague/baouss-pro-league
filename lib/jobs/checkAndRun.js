...
import { isSafeToComputeNow, isSafeForFinalScores } from "../prizes/gwTiming";
...
async function processJobType(jobType, gw, fixtures, today, eventStatusData, bootstrapEvents) {
  ...
  // Same combined gate as LMS: bonus confirmation and auto-substitution
  // are separate FPL processes, and auto-subs can lag behind bonus
  // confirmation. Locking in a "final" number before both are true risks
  // freezing a score that's about to change once an auto-sub is applied.
  const shouldLock = isGwFinalizedFromStatus(eventStatusData, gw, bootstrapEvents) && isSafeForFinalScores(fixtures);
