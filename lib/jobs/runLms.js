...
import { fpl } from "../fpl";
import { resolveByPointsAndBench, resolveByCaptainOrRandom, buildRound2Pool, LMS_START_GW, eliminationsThisWeekFor } from "../prizes/lms";
import { isGwFullyFinalized } from "../prizes/liveScores";
import { isSafeForFinalScores } from "../prizes/gwTiming";
...
export async function runLmsForGw(gwNum) {
  ...
  // Confirmed the real gap: bonus confirmation and auto-substitution are
  // two SEPARATE FPL processes, and auto-subs can genuinely lag behind
  // bonus confirmation - "typically the morning after the last match,"
  // per FPL's own documentation. A manager's real score depends on which
  // 11 players actually counted, so this is a second, independent check,
  // not a substitute for the one above - both have to be true before an
  // irreversible elimination runs.
  const gwFixtures = await fpl.fixtures(gwNum);
  if (!isSafeForFinalScores(gwFixtures)) {
    return {
      ok: false,
      status: "gw_not_finished",
      message: `GW${gwNum}'s bonus points are confirmed, but not enough time has passed for FPL's automatic substitutions to have settled yet - LMS refuses to run until it has. Nobody was eliminated.`,
    };
  }

  const round = gwNum >= 25 ? 2 : 1;

  // The festive-period risk, closed here: gameweeks are normally strictly
  // sequential in time, so this never matters in a normal week - but a
  // postponed/rescheduled match (common exactly when cup clashes compress
  // the calendar, like Christmas) could mean an earlier gameweek's true
  // last match airs AFTER a later gameweek's matches have already
  // finished. Without this check, that later gameweek could get decided
  // first, using a "who's still alive" list that doesn't yet reflect
  // whoever's about to be eliminated from the earlier one. This enforces
  // strict order: refuse to touch gwNum until every earlier started
  // gameweek (within the same round, skipping the break weeks) already
  // has its own elimination(s) recorded.
  const earlierGwsToCheck = [];
  for (let g = LMS_START_GW; g < gwNum; g++) {
    if (LMS_BREAK_GWS.includes(g)) continue;
    if (round === 1 && g >= 25) continue; // round 2 gws don't block round 1 ordering
    const gEvent = bootstrap.events.find((e) => e.id === g);
    if (gEvent && new Date(gEvent.deadline_time) <= new Date()) {
      earlierGwsToCheck.push(g);
    }
  }
  for (const earlierGw of earlierGwsToCheck) {
    const earlierRound = earlierGw >= 25 ? 2 : 1;
    const { data: earlierRows } = await supabaseAdmin
      .from("lms_eliminations")
      .select("entry_id")
      .eq("round", earlierRound)
      .eq("gw_eliminated", earlierGw);
    const expectedCount = eliminationsThisWeekFor(earlierGw);
    if (!earlierRows || earlierRows.length < expectedCount) {
      return {
        ok: false,
        status: "gw_not_finished",
        message: `GW${gwNum} can't run yet - GW${earlierGw} hasn't had its own elimination(s) recorded first. LMS processes gameweeks strictly in order.`,
      };
    }
  }

  // The actual fix: without this, a manual click followed by an
  // automated run (or two automated runs on different days, since the
  // daily-recompute logic has no inherent memory of "this gameweek is
  // already done") would each independently ask "who's currently
  // remaining" and eliminate someone new - producing two eliminations
  // for a week that should only ever have one. This checks "has THIS
  // gameweek already had its elimination recorded at all" as its own
  // explicit gate, completely separate from "who's left" - once GW2 has
  // any elimination row for this round, nothing can add another one,
  // regardless of what triggers the call or how many times it's tried.
  const { data: existingForThisGw } = await supabaseAdmin
    .from("lms_eliminations")
    .select("entry_name")
    .eq("round", round)
    .eq("gw_eliminated", gwNum);
  if (existingForThisGw && existingForThisGw.length > 0) {
    return {
      ok: true,
      status: "already_done",
      message: `GW${gwNum} already has its elimination(s) recorded (${existingForThisGw.map((r) => r.entry_name).join(", ")}) - not running again.`,
    };
  }

  const eliminationsThisWeek = eliminationsThisWeekFor(gwNum);
  ...
  for (const m of remaining) {
    ...
    // The confirmed bug, fixed directly here since this function reads
    // entryHistory itself rather than through loadAllHistories: FPL's
    // own per-gameweek points field is GROSS, hits not deducted. An
    // elimination decision has to be based on the real, net score.
    const netPoints = row ? row.points - (row.event_transfers_cost || 0) : null;
    scoresByEntry.set(m.entry, { points: netPoints, benchPoints: row ? row.points_on_bench : null });
  }
