...
import { gwStatus } from "../prizes/liveScores";
...
export async function runLmsForGw(gwNum) {
  ...
  if (gwStatus(event) !== "completed") {
    return {
      ok: false,
      status: "gw_not_finished",
      message: `GW${gwNum} hasn't fully finished yet (bonus points not yet confirmed by FPL) - LMS refuses to run until it has. Nobody was eliminated.`,
    };
  }

  const round = gwNum >= 25 ? 2 : 1;
  const eliminationsThisWeek = round === 1 ? ROUND1_ELIMINATIONS_PER_WEEK : ROUND2_ELIMINATIONS_PER_WEEK;
