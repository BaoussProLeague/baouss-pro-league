...
// EXCEPT for an activation that falls on the currently in-progress
// gameweek, since history doesn't have that row populated yet - that's
// the "0 points" bug. `liveScoresMap`, when provided, patches exactly
// that one case with a real live-computed score.
...
export function chipPrizes(histories, currentGw = null, liveScoresMap = null, genuinelyLive = false) {
  const results = {};
  ...
  for (const chipKey of Object.keys(CHIP_NAMES)) {
    ...
    for (const m of histories) {
      ...
      for (const act of activations) {
        let score;
        if (act.event === currentGw && liveScoresMap && liveScoresMap.has(m.entry)) {
          score = liveScoresMap.get(m.entry).points;
        } else {
          const gwRow = m.history.find((h) => h.event === act.event);
          if (!gwRow) continue;
          score = gwRow.points; // includes hits, per your rules doc
        }
      ...
      if (best) {
        contenders.push({
          ...
          // The bug: this used to just check "was this the gameweek we
          // last had data for" - which stays true for days after that
          // gameweek's matches actually finished, since currentGw
          // doesn't advance until the NEXT gameweek starts. genuinelyLive
          // requires an actual match to be in progress right now, not
          // just "this happens to be the most recent gameweek."
          isLive: best.gw === currentGw && genuinelyLive,
        });
      }
    }
