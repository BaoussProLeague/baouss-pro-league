...
// The confirmed bug: event_total was trusted unconditionally, with no
// check for whether the target gameweek has actually started scoring
// yet. Right at the gap between one gameweek's deadline and its first
// match kicking off, FPL's own event_total field can still be carrying
// over the PREVIOUS gameweek's number rather than genuinely reflecting
// 0 for the new one - proven by your own report of seeing old scores
// between deadline and kickoff. `fixturesStarted` should be true only
// once at least one match from the target gameweek has actually begun -
// if nothing has, every score is forced to 0, which is what's actually
// true at that point, rather than trusting a field that hasn't caught
// up yet.
// The same confirmed gross-vs-net issue applies here: event_total is the
// live equivalent of entry_history's per-gameweek points field, and by
// the same logic almost certainly carries the identical "gross, hit not
// deducted" convention - Classic standings has no separate hit-cost
// field alongside event_total to correct it with directly, so an
// optional pre-fetched hitCostsMap (see getLiveHitCostsFromPicks below)
// is subtracted here when supplied.
export function getLiveGwScoresFromStandings(classicEntries, fixturesStarted = true, hitCostsMap = null) {
  return classicEntries.map((e) => {
    const gross = fixturesStarted ? (e.event_total ?? 0) : 0;
    const hit = hitCostsMap?.get(e.entry) ?? 0;
    return {
      entry: e.entry,
      entryName: e.entry_name,
      points: gross > 0 ? gross - hit : gross,
    };
  });
}

// The hit cost for the CURRENT gameweek isn't exposed anywhere in the
// Classic standings response - it only exists in each manager's own
// picks data (entry_history.event_transfers_cost), known at deadline
// and unchanging afterward. This is the one extra per-manager fetch
// needed to correct live scores the same way the confirmed history fix
// corrects finalized ones - only meaningfully used while a gameweek is
// actually in progress, same cost profile as the existing bench-points
// live fetch.
export async function getLiveHitCostsFromPicks(entries, gw) {
  const results = await Promise.all(
    entries.map(async (e) => {
      try {
        const picks = await fpl.entryPicks(e.entry, gw);
        return { entry: e.entry, hitCost: picks.entry_history?.event_transfers_cost || 0 };
      } catch {
        return { entry: e.entry, hitCost: 0 };
      }
    })
  );
  return new Map(results.map((r) => [r.entry, r.hitCost]));
}
