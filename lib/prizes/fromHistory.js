import { fpl } from "../fpl";
import { isGwFinalizedFromStatus } from "./liveScores";
...
export async function loadAllHistories(entries) {
  ...
  const histories = await Promise.all(
    entries.map(async (e) => {
      ...
      // The confirmed bug, fixed once here at the source rather than
      // patched in every individual consumer: FPL's own per-gameweek
      // `points` field is GROSS - it does NOT have that gameweek's own
      // transfer-hit cost subtracted (the hit only reduces the
      // cumulative total_points). Confirmed via FPL's own community and
      // independently by a separate project's own verified bug fix, not
      // assumed. Every consumer of `.history` here on out gets the real,
      // net score automatically - grossPoints is kept alongside it for
      // anywhere that specifically wants to show the raw score and the
      // hit separately (e.g. a team's hit history).
      const correctedHistory = h.current.map((row) => ({
        ...row,
        grossPoints: row.points,
        points: row.points - (row.event_transfers_cost || 0),
      }));
      return { ...e, history: correctedHistory, chips: h.chips, pastSeasons: h.past };
    })
  );
  return histories;
}
...
export function rankJumpByMonth(histories, monthGwMap, events, currentGw = null, liveScoresMap = null, eventStatusData = null) {
  ...
  for (let i = 0; i < months.length; i++) {
    ...
    const baselineLocked = baselineEvent && isGwFinalizedFromStatus(eventStatusData, baselineGw, events);
    if (!baselineLocked) {
      monthlyResults[month] = [];
      continue;
    }
    ...
    const scored = histories
      ...
      .filter(Boolean)
      .filter((r) => r.jump > 0); // only managers who actually moved up are eligible
    scored.sort((a, b) => b.jump - a.jump);
    monthlyResults[month] = scored;
  }

  // Format like MOTM: completed months (locked) + current month leader
  const rankJumpCompletedMonths = Object.entries(monthlyResults)
    .filter(([, managers]) => managers.length > 0)
    .map(([month, managers]) => ({
      month,
      winner: managers[0], // highest jumper
      jump: managers[0].jump,
    }));

  const currentMonthKey = currentGw ? Object.entries(monthGwMap).find(([, gws]) => gws.includes(currentGw))?.[0] : null;
  let currentMonthRankJumpLeader = null;
  if (currentMonthKey && monthlyResults[currentMonthKey]?.length > 0) {
    const leader = monthlyResults[currentMonthKey][0];
    currentMonthRankJumpLeader = {
  ...
  return { rankJumpCompletedMonths, currentMonthRankJumpLeader };
}
