...
export function benchPoints(histories, asOfGw = null, currentGw = null, liveBenchMap = null) {
  const totals = histories.map((m) => {
    ...
    if (currentGw && liveBenchMap && liveBenchMap.has(m.entry) && !eligible.some((h) => h.event === currentGw)) {
...
export function firstToThreshold(histories, threshold, currentGw = null, liveScoresMap = null) {
    ...
    .map((m) => {
      ...
      if (currentGw && liveScoresMap && liveScoresMap.has(m.entry)) {
        ...
        if (liveTotal >= threshold) {
          return { entry: m.entry, entryName: m.entryName, gwReached: currentGw, totalAtHit: liveTotal, isLive: true };
        }
      }
      return null;
    })
...
export function rankJumpByMonth(histories, monthGwMap, currentGw = null, liveScoresMap = null) {
  const results = {};
  for (const [month, gws] of Object.entries(monthGwMap)) {
    const firstGw = Math.min(...gws);
    const lastGw = Math.max(...gws);
    const startRanks = miniLeagueRankAtGw(histories, firstGw - 1 >= 1 ? firstGw - 1 : firstGw, currentGw, liveScoresMap);
...
export function wildcardVision(histories, currentGw = null, liveScoresMap = null) {
  ...
  for (const m of histories) {
    ...
    for (const act of activations) {
      ...
      if (currentGw && windowGws.includes(currentGw) && liveScoresMap && liveScoresMap.has(m.entry) && !rows.some((r) => r.event === currentGw)) {
