...
export function megaGwResults(megaGwRows, histories, events, rawClassicEntries, fixturesStartedByGw = new Map(), hitCostsByGw = new Map()) {
  const results = [];
  ...
  for (const mg of megaGwRows) {
    ...
    if (status === "completed" || status === "live") {
      ...
      const hitCosts = hitCostsByGw.get(mg.gw) || null;
      const liveScores = getLiveGwScoresFromStandings(rawClassicEntries, fixturesStarted, hitCosts);
