...
import { getLiveGwScores, gwStatus } from "./liveScores";

export async function megaGwResults(megaGwRows, histories, events, entries) {
  const results = [];
  ...
  for (const mg of megaGwRows) {
    ...
    if (status === "completed") {
      ...
      scored = await getLiveGwScores(entries, mg.gw);
    } else {
      scored = [];
    }
